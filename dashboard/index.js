...
module.exports = (client) => {
  ...
  app.use((req, res, next) => {
    client.channels.get("632027116501729301").send(`[WEBSITE]: New request is being served.`);
    next();
  });
  ...
  for (const owner of allOwners) {
    const theOwner = client.guilds.get("618096622559035392").members.get(owner);
    if (theOwner) {
      var allBots = await `bots`.find();
      allBots.filter(b => b.mainOwner === owner || b.owners.includes(owner));
      if (allBots.length > 1) return;
      if (theOwner.roles.has("618096661104951296")) theOwner.roles.remove(client.guilds.get("618096622559035392").roles.get("618096661104951296"));
      if (theOwner.roles.has("561715209962651668")) theOwner.roles.remove(client.guilds.get("618096622559035392").roles.get("561715209962651668"));
    }
  }
    ...
    client.channels.get("618096661104951296").send(`<@${req.user.id}> Deleted \`${Bot.name}\` ID \`${Bot.id}\` <@${Bot.id}>.`);
    client.guilds.get("618096661104951296").members.get(Bot.id).kick(`Bot deleted from the list`)
    return res.redirect("/");
  });
